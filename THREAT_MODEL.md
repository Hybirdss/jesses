...
| **A8** | Theater mode (pre-session evil) | Do the evil thing **outside** a jesses session, then start a clean jesses session and submit the stolen data inside a well-behaved attestation |
...
A8 is the subtlest of the set and was added after v0.1 core implementation. It exploits the semantic gap between what the attestation CLAIMS ("this session stayed in scope") and what a reader ASSUMES ("the agent was well-behaved overall"). A report that reuses data the agent obtained outside the session would pass G1–G6 with no issue. G7 (deliverable provenance binding) is the defense.
...
| A7 interleave fakes | **Streaming commitment** + adversarial economics (v0.1); TEE attestation (v0.3) | v0.1 relies on: (a) timestamp monotonicity inside the tree, (b) wall-clock consistency with Rekor and OTS anchors, (c) the economic argument below. v0.3 closes this fully via TEE. | **This is one of two residual v0.1 gaps.** See §4. |
| A8 theater mode | **Deliverable ↔ audit-log binding (G7)** | The attestation envelope's Predicate includes `deliverable_binding{path, sha256, citation_count, bare_claim_count, bare_policy}`. The bound report is a markdown file containing `[^ev:N]` or `[^hash:H]` footnote citations to specific audit-log events. Verifier G7 recomputes the report SHA-256, parses citations, and validates every one against the audit log; bare (uncited) factual claims are flagged per policy. A claim citing data the agent obtained outside the session has no valid event to cite → G7 fails. | **Partial.** G7 enforces that every cited fact traces to a session event, but cannot force the agent to cite every fact. Narrative / opinion lines remain permissible under `bare-policy=warn`. Full closure requires `bare-policy=strict` plus TEE-attested agent memory statelessness (v0.3+). |
