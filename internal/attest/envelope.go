...
type Predicate struct {
	...
	// DeliverableBinding ties this envelope to a specific report
	// file. When set, verifier G7 reads the file, checks the hash
	// matches, and validates every [^ev:N] / [^hash:H] citation.
	// When nil, G7 is advisory (no report submitted alongside
	// envelope).
	DeliverableBinding *DeliverableBinding `json:"deliverable_binding,omitempty"`
}

// DeliverableBinding binds an attestation envelope to one report
// file produced during the session. The binding defends against the
// A8 "theater mode" attack: an attacker who runs evil actions
// outside a jesses session and then replays a clean session to
// submit the stolen data. With binding, every factual claim in the
// report must cite a specific audit-log event — uncited facts are
// flagged, hash-mismatched citations fail verification.
type DeliverableBinding struct {
	// Path is the report file path relative to the envelope
	// location. Usually "report.md".
	Path string `json:"path"`

	// SHA256 is the hex-encoded SHA-256 of the report bytes at the
	// moment of binding. Verifier G7 re-hashes the file on disk
	// and compares.
	SHA256 string `json:"sha256"`

	// CitationCount is how many [^ev:N] / [^hash:H] markers the
	// report contained at binding time.
	CitationCount int `json:"citation_count"`

	// BareClaimCount is how many lines carried factual markers
	// (URLs, paths, IPs) without any citation. Policy below decides
	// whether bare claims fail the verification.
	BareClaimCount int `json:"bare_claim_count"`

	// BarePolicy dictates how G7 treats bare claims:
	//   "allow"  — no effect on verdict
	//   "warn"   — advisory, does not fail verdict
	//   "strict" — fails G7 when any bare claim exists
	BarePolicy string `json:"bare_policy"`
}
...
// the Statement, signs it, and returns the DSSE-ish envelope with
// no deliverable binding. This is the baseline path for sessions
// that do not produce a bound report.
func Build(f session.Finalized) (Envelope, error) {
	return BuildWithBinding(f, nil)
}

// BuildWithBinding is Build with an explicit DeliverableBinding
// inserted into the Predicate. cmd/jesses calls this when the hook
// was invoked with --report; the resulting envelope commits to the
// report's hash as part of the signed payload, so verifier G7 can
// later re-check that the report bytes on disk match what was
// attested. Pass nil binding to get the same result as Build.
func BuildWithBinding(f session.Finalized, binding *DeliverableBinding) (Envelope, error) {
	stmt := Statement{
		Type: StatementType,
		Subject: []Subject{{
			Name:   f.SessionID,
